...
This is known as the \textbf{Factor Theorem}.
...
\begin{enumerate}
  ...
  \item The graph of a quartic function is shown below. Identify all zeros, state whether each has odd or even multiplicity, and describe the end behavior.
  \begin{diagramdata}
  {
    "type": "polynomial-curve",
    "shape": "cross-cross-touch",
    "zeros": [
      {"x": -2, "label": "-2", "behavior": "cross"},
      {"x": 0, "label": "0", "behavior": "cross"},
      {"x": 3, "label": "3", "behavior": "touch"}
    ]
  }
  \end{diagramdata}
...
\begin{enumerate}
  ...
  \(\boxed{\text{Touches at } x = 4, -2;\ \text{crosses at } x = 1}\)
...
\begin{enumerate}
  ...
  Reading the graph: it crosses straight through at \(x = -2\) and \(x = 0\), so both have odd multiplicity. It touches and turns around at \(x = 3\), so that zero has even multiplicity.

  A possible equation using the smallest multiplicities consistent with the graph:
  \[
  f(x) = x(x + 2)(x - 3)^2
  \]
  where \((x - 3)^2\) gives the touch at \(x = 3\), and the overall degree is even with a positive leading coefficient, matching both ends up.

  \(\boxed{\text{Zeros: } x = -2 \text{ (odd)},\; 0 \text{ (odd)},\; 3 \text{ (even)}. \text{ End behavior: both ends up}}\)
...
\ReflectionPage{Unit 7, Topic 1: Exponential Growth and Decay}{https://www.scotchegg.co}
