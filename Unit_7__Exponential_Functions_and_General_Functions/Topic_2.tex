...
Early on, a linear function might appear to grow faster, but eventually an exponential function surpasses it because it compounds multiplicatively. For example, \(y = 2x + 5\) starts out above \(y = 2^x\), but \(2^x\) overtakes it between \(x = 3\) and \(x = 4\):
...
\begin{diagramdata}
{
  ...
  "xMax": 5,
  "series": [
    {"label": "y = 2x + 5 (linear)", "fn": "2*x+5", "color": "#1E3A2F", "dash": false},
    {"label": "y = 2^x (exponential)", "fn": "Math.pow(2,x)", "color": "#B08840", "dash": true}
  ]
}
\end{diagramdata}
...
A graph shows a straight line with constant slope and an exponential curve that starts below the line but rises faster, overtaking it around \(x = 5\).
...
\begin{diagramdata}
{
  ...
  "xMax": 8,
  "series": [
    {"label": "L(x) = 100 + 20x (linear)", "fn": "100+20*x", "color": "#1E3A2F", "dash": false},
    {"label": "E(x) = 100(1.15)^x (exponential)", "fn": "100*Math.pow(1.15,x)", "color": "#B08840", "dash": true}
  ]
}
\end{diagramdata}
...
\begin{enumerate}
  ...
  \item The exponential curve overtakes the line at the intersection point, around \(x = 5\).
...
\ReflectionPage{Unit 7, Topic 3: Function Notation and Evaluation}{https://www.scotchegg.co}
